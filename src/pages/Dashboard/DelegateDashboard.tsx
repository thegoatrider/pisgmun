...
export const DelegateDashboard: React.FC = () => {
  ...
  const commObj = committees.find((c) => c.id === assignedCommId);
  ...
  const commPdfPath = pdfMapping[assignedCommId?.toLowerCase()] || '#';
          ...
          <Card elevation="md" style={{ padding: '2rem' }}>
            ...
            {isPending && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', color: 'var(--color-warning)', backgroundColor: 'var(--color-warning-bg)', border: '1px solid var(--color-warning)', borderRadius: 'var(--radius-md)', padding: '1rem 1.25rem', fontSize: '0.88rem' }}>
                ...
                <div>
                  <strong>Pending Verification:</strong> Your registration form is under review. Your assigned country and committee will unlock here as soon as the coordinator approves your file.
                </div>
              </div>
            )}
            ...
            {isApproved ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', backgroundColor: 'var(--color-bg-main)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '1.25rem' }}>
                  ...
                  <div>
                    ...
                    <strong style={{ fontSize: '1.2rem', color: 'var(--color-primary)' }}>
                      {assignedCommId.toUpperCase()}
                    </strong>
                  </div>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.88rem' }}>
                  <div><strong>Council:</strong> {commObj?.name}</div>
                  <div><strong>Grade Target:</strong> Grade {commObj?.grade} eligibility</div>
                  <div><strong>Agenda:</strong> {commObj?.agenda}</div>
                  {commObj && (
                    <Button
                      variant="outline"
                      size="sm"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 1.5rem', border: '2px dashed var(--color-border)', borderRadius: 'var(--radius-md)', textAlign: 'center', gap: '0.75rem' }}>
                ...
                <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--color-text-muted)', lineHeight: '1.4' }}>
                  Country and Committee allocations will display here once your application is APPROVED by administrative staff.
                </p>
              </div>
            )}
          </Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <Card elevation="md" style={{ padding: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', padding: '1rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', backgroundColor: isApproved ? 'var(--color-bg-main)' : 'rgba(226, 232, 240, 0.4)', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  {isApproved ? (
                    <FileText size={20} style={{ color: 'var(--color-secondary)', flexShrink: 0 }} />
                  ) : (
                  ...
                  <div>
                    <span style={{ fontWeight: 700, fontSize: '0.85rem', display: 'block', color: isApproved ? 'var(--color-primary)' : 'var(--color-text-muted)' }}>
                      Background Guide
                    </span>
                    <span style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)' }}>
                      {isApproved ? `${assignedCommId.toUpperCase()} Briefing Manual` : 'Unlocks on Approval'}
                    </span>
                  </div>
                </div>
